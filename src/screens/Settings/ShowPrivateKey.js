...
export const ShowPrivateKey = () => {
	...
	React.useEffect(() => {
		if (dataUser.length && currentAccount !== '') {
			setPhrase(
				dataUser.find((item) => item.name === currentAccount).privateKey
			)
		}
	}, [dataUser])
	...
	return (
		<ScrollView
			...
			<Header title='Private Key' />
			<View style={{ flex: 1, marginBottom: 40 }}>
				<WalletText style={{ marginBottom: 30 }}>
					If you ever change browsers or move computers, you will need this
					private key to access your accounts. Save them somewhere safe and
					secret.
				</WalletText>
				<Alert title='Keep Private Key Safe!'>
					Do not share this private key with anyone! These words can be used to
